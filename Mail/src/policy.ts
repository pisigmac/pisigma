...
export function parseAllowedFrom(raw: string): string[] {
  try {
...
export function pickFromAddress(
  ...
  const allowed = parseAllowedFrom(product.allowed_from)
  if (requested) {
    if (allowed.length === 0 || allowed.includes(requested) || allowed.includes('*')) {
      return { ok: true, from: requested }
    }
    return {
      ok: false,
      error: `from address not allowlisted for product ${product.slug}`,
    }
  }
  if (allowed.length > 0 && allowed[0] !== '*') {
    return { ok: true, from: allowed[0]! }
  }
  return { ok: true, from: defaultFrom }
}
